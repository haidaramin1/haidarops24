...
Files get copied to etc/nginx/conf.d/https.conf

What does the output look like the second time you run this playbook?
It changes to :false, because the file already exists, which is idempotency 
...
1. So first of all, we need to install nginx on the webserver 
2. We need to update the 05 web yml file to include the restart of the nginx service 
3. Since it's authenticating to itself, it cannot establish a connection without --insecure 
...
It can ruin the connection and interrupt ongoing connections
