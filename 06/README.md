...
* If you want to serve files under a non-standard directory (such as the one we create above), we must
  ...
  So here we use ansible.builtin.file module to create the directory, and then builtin.copy to copy it and insert it to the directory. 
...
Yes, it's good. Because then we can control the flow and only restart when necessary. 
...
Ansible, but in general terms.0
