...
We add: state: started 

This ensures that nginx is started, not just enabled at boot. It's also idempotent which is that it wont run again if not necessary (It's already online) 
...
This makes it have an elevated privelege, so it applies to all tasks, it'll run as sudo-user now without needing become: true.
...
We can't just remove it, first we have to stop it - "state: stopped" 
then we have to disable it from starting at boot - "enabled: false" 
then we can uninstall the package - "state: absent" 
